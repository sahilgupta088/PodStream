...
const Upload = ({ setUploadOpen }) => {
    ...
    const uploadFile = async (file, index) => {
        ...
        const publicUrl = "https://ocivkhdmoipyzdkgnlwd.supabase.co/storage/v1/object/public/PODSTREAM/" + fileName.split('').map(char => (char === ' ' ? '%20' : char)).join('');
        
        console.log(publicUrl)
        
    
        const { error: dbError } = await supabase.from('PODSTREAM').insert({
            fileName,
